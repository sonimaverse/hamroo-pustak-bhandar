...
export class OrderController {
  /**
   * POST /api/orders
   * Checkout: Create order from user's current cart
   */
  static async checkout(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const order = await OrderService.checkout(req.user._id.toString(), validatedInput);
  ...
  /**
   * GET /api/orders/:id
   * Get order by ID (security check: owner or admin)
   */
  static async getOrderById(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const { id } = req.params;
      const order = await OrderService.getOrderById(req.user._id.toString(), req.user.role, id);
