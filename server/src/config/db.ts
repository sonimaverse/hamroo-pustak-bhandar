...
export const connectDB = async (): Promise<void> => {
  ...
  if (isPlaceholderUri) {
    if (isProduction) {
      throw new Error(
        'FATAL: MONGODB_URI environment variable is required and must be properly configured in production mode. ' +
        'Fallback in-memory storage is strictly disabled in production.'
      );
    }
    console.warn(
  ...
  try {
    ...
    if (isProduction) {
      throw new Error(
        `FATAL: Could not connect to MongoDB Atlas in production: ${error.message}. ` +
        'Fallback in-memory storage is strictly disabled in production.'
      );
    }
