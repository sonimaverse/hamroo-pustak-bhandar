...
const storeInstance = new FallbackStore();

export const fallbackStore = new Proxy(storeInstance, {
  get(target: any, prop: string | symbol, receiver: any) {
    if (process.env.NODE_ENV === 'production') {
      throw new ApiError(
        500,
        'FATAL: Fallback in-memory storage engine is strictly disabled in production mode. Real MongoDB connection is required.'
      );
    }
