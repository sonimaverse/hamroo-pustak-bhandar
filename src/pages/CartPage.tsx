...
import { ShoppingCart, Trash2, ArrowRight, ArrowLeft, BookOpen, ShieldCheck } from 'lucide-react';

export const CartPage: React.FC = () => {
  const { cart, loading, error, updateQuantity, removeItem, clearCart, totalAmount, itemCount } = useCart();
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-3xl border border-stone-200 p-6 sm:p-8 shadow-2xs flex items-center justify-between">
        ...
        {!isEmpty && (
          <button
            onClick={clearCart}
            className="text-xs font-bold text-stone-500 hover:text-red-700 flex items-center gap-1 transition"
          >
      ...
      {isEmpty ? (
        <div className="bg-white rounded-3xl border border-stone-200 p-12 text-center my-6 space-y-4">
          ...
          <p className="text-xs text-stone-500 max-w-md mx-auto">
            You haven't added any books to your cart yet. Explore our extensive catalog of literature and educational guides.
          </p>
          <div className="pt-2">
            <Link
              to="/shop"
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-16 h-20 bg-stone-100 rounded-xl flex items-center justify-center shrink-0 overflow-hidden">
                    {item.coverImage ? (
                      <img src={item.coverImage} alt={item.title} className="h-full object-contain" />
                    ) : (
                      <BookOpen className="w-8 h-8 text-stone-400 stroke-1" />
                    )}
                  </div>
                  <div>
                    <h3 className="font-serif font-bold text-stone-900 text-sm">{item.title}</h3>
                    <p className="text-xs text-stone-500 font-medium">by {item.author}</p>
                    <div className="text-xs font-bold text-stone-900 mt-1">
                      Rs. {item.unitPrice.toLocaleString()}{' '}
                      {item.wholesalePrice && item.wholesalePrice < item.regularPrice && (
                        <span className="text-[10px] text-emerald-700 font-bold ml-1">(Wholesale Rate)</span>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between w-full sm:w-auto gap-4 pt-2 sm:pt-0 border-t sm:border-t-0 border-stone-100">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="p-2 text-stone-400 hover:text-red-700 hover:bg-red-50 rounded-xl transition"
                    title="Remove Item"
                  >
          ...
          <div className="lg:col-span-4 bg-white rounded-3xl border border-stone-200 p-6 shadow-2xs h-fit space-y-4">
            ...
            <div className="space-y-2 text-xs">
              <div className="flex justify-between text-stone-600">
                ...
                <span className="font-bold text-stone-900 font-sans">Rs. {totalAmount.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-stone-600">
                <span>Estimated Shipping (Nepal)</span>
            ...
            <div className="flex items-center gap-2 text-[11px] text-stone-500 pt-2 border-t border-stone-100">
              ...
              <span>Official Tax Invoicing & Delivery Guarantee across Nepal.</span>
            </div>
          </div>
