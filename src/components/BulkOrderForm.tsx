...
type DeliveryType = 'home' | 'pickup' | 'office';

export default function BulkOrderForm({ isOpen, onClose }: BulkOrderFormProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-card">
        <div className="sticky top-0 flex items-center justify-between border-b border-brown-100 bg-white px-6 py-4">
          <div>
            <h3 className="font-serif text-xl font-bold text-brown-800">Bulk Order Request</h3>
        ...
        {submitted ? (
          <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
            ...
            <p className="mt-4 font-serif text-xl font-bold text-brown-800">Request Submitted!</p>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  { value: 'office', label: 'Office Delivery', icon: '🏢' },
                ].map((option) => (
                  <button
                    key={option.value}
                    type="button"
                    onClick={() => updateField('deliveryType', option.value)}
                    className={`flex flex-col items-center gap-2 rounded-lg border-2 px-4 py-3 text-xs font-bold transition ${
                      formData.deliveryType === option.value
                        ? 'border-gold-400 bg-gold-50 text-brown-900'
                        : 'border-brown-200 text-brown-600 hover:border-gold-400'
                    }`}
                  >
