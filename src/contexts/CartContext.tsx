  ...
  const syncGuestCartToBackend = async () => {
    ...
    if (guestCart.items && guestCart.items.length > 0) {
      for (const item of guestCart.items) {
        try {
          await cartService.addItem(item.bookId, item.quantity);
        } catch (err) {
          console.error(`Failed to sync guest cart item ${item.bookId}:`, err);
        }
      }
  ...
  const addItem = async (bookId: string, quantity: number) => {
    try {
      ...
      if (isAuthenticated && token) {
        ...
        const currentCart = cart || loadGuestCartFromStorage();
        ...
        const unitPrice = book.effectivePrice ?? book.price ?? book.regularPrice;

        if (existingIndex > -1) {
          const currentQty = newItems[existingIndex].quantity;
          const newQty = currentQty + quantity;
          if (newQty > book.stockQuantity) {
            throw new Error(`Insufficient stock. Requested ${newQty}, available ${book.stockQuantity}.`);
          }
          ...
          if (quantity > book.stockQuantity) {
            throw new Error(`Insufficient stock. Requested ${quantity}, available ${book.stockQuantity}.`);
          }
  ...
  const updateQuantity = async (bookId: string, quantity: number) => {
    try {
      ...
      if (isAuthenticated && token) {
        ...
        const item = currentCart.items[existingIndex];
        if (quantity > item.stockQuantity) {
          throw new Error(`Insufficient stock. Requested ${quantity}, available ${item.stockQuantity}.`);
        }

        let newItems = [...currentCart.items];
        if (quantity <= 0) {
          newItems = newItems.filter(i => i.bookId !== bookId);
        } else {
          newItems[existingIndex] = {
            ...item,
            quantity,
  ...
  const removeItem = async (bookId: string) => {
    try {
      ...
      if (isAuthenticated && token) {
        ...
        const currentCart = cart || loadGuestCartFromStorage();
        const newItems = currentCart.items.filter(item => item.bookId !== bookId);
        const totalAmount = newItems.reduce((sum, i) => sum + i.subtotal, 0);
